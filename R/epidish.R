...
epidish <- function(avdata.m,ref.m,method=c("RPC","CBS","CP"), maxit=50,
                    nu.v=c(0.25,0.5,0.75), constraint="inequality"){
    if(method=="RPC"){
...
DoCBS <- function(avdata.m,ref.m,nu.v=c(0.25,0.5,0.75)){
    ...
    for(nu in nu.v){
     ...
     rownames(est.m) <- colnames(data2.m);    
     for(s in 1:ncol(data2.m)){    
      svm.o <- svm(x=ref2.m,y=data2.m[,s],scale = TRUE,     
                   type="nu-regression", kernel ="linear", nu = nu);    
...
return(list(estF=westQP.m,ref=ref.m[rep.idx,],    
            dataREF=avdata.m[map.idx[rep.idx],]))}    
